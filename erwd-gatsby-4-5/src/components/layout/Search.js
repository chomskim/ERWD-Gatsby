...
const Search = ({ searchIndex }) => {
  ...
  return (
    <div className='relative w-64 text-gray-600'>
      ...
      {results.length > 0 && (
        <div className='flex flex-col border-4 border-blue-500 absolute z-30 top-0 w-64 left-0 mt-12 bg-white rounded'>
          {results.slice(0, searchResultSize).map(({ title, description, path }) => (
            <Link key={path} to={path} className='hover:bg-blue-100 w-full p-1 border-b border-gray-200'>
              <p>{title}</p>
              <p className='text-xs'>{description}</p>
            </Link>
          ))}
          {results.length > searchResultSize && (
            <Link to={`/search?q=${query}`} className='hover:bg-blue-100 w-full p-1'>
              <p className='text-xs'>+ {results.length - searchResultSize} more</p>
            </Link>
          )}
        </div>
      )}
